...
function openWindow(){processLogs(false).then(function(){var players=[];$.each(friends,function(pid){players.push({'id':pid,'timeUntilReady':timeUntilSesReady(pid)});});players.sort(function(a,b){return a.timeUntilReady-b.timeUntilReady;});var windowContent=new west.gui.Scrollpane();var friendsTable=new west.gui.Table();friendsTable.addColumn('remove-link');friendsTable.addColumn('player-names').appendToCell('head','player-names','<img src="//westzzs.innogamescdn.com/images/icons/user.png" alt="" />&nbsp;'+'Name');friendsTable.addColumn('total-received').appendToCell('head','total-received','Received');friendsTable.addColumn('send-links').appendToCell('head','send-links','Send');for(var i=0;i<players.length;i++)

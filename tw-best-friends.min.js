...
// @run-at          document-end
// ==/UserScript==
var script=document.createElement('script');script.type='text/javascript';script.textContent='('+(function(){var friends={};var lastSent={};var logTypes={"friendDrop":"Friends","jobDrop":"Jobs","battleDrop":"Fort Battles","adminDrop":"Admin intervention","wofPay":"Used","duelDrop":"Duels","duelNPCDrop":"NPC Duels","adventureDrop":"Adventures","questDrop":"Quests","itemUse":"Used items","buildDrop":"Construction"};var newLogs=true;var logsMetadata=null;var playerLogs=null;var dropTypeLogs=null;var logsLocked=false;var sortingDirection="Up";function getActiveSesKeys(){return Object.keys(Game.sesData);}
...
function getSesSmallestTimer(){var count=0;var smallestTimer=0;$.each(friends,function(playerId,client){if(count==0)
...
function processLogs(background){if(logsLocked)throw new Error("Please don't try and process the logs twice at the same time.");if(!newLogs)return Promise.resolve();logsLocked=true;loadLogs();var sesKey=getActiveSesKeys()[0];if(typeof(logsMetadata.logTypes)=='undefined')
logsMetadata.logTypes={};if(sesKey!==logsMetadata.sesKey){playerLogs={};dropTypeLogs={};logsMetadata.sesKey=sesKey;}
...
if(entry.details=="deduct"){entry.value*=-1;}
dropTypeLogs[entry.type]=(dropTypeLogs[entry.type]||0)+ +entry.value;if(typeof(logsMetadata.logTypes[entry.type])=='undefined'){logsMetadata.logTypes[entry.type]=entry.description;}
if(entry.type==='friendDrop'){var senderId=JSON.parse(entry.details).player_id;if(playerLogs.hasOwnProperty(senderId)){playerLogs[senderId].total+=+entry.value;playerLogs[senderId].frequency.push(entry.date);}else{playerLogs[senderId]={total:+entry.value,frequency:[entry.date]};}}})&&data.hasNext;if(background){setTimeout(callback,1000);}else{callback();}});}
...
table.appendRow().appendToCell(-1,'remove-link',generateDeleteFriendLink(pid)).appendToCell(-1,'player-names',generatePlayerLink(pid)).appendToCell(-1,'total-received',logToolTip);if(timeUntilSesReady(pid)){var totalSec=timeUntilSesReady(pid);var hours=parseInt(totalSec / 3600)%24;var minutes=parseInt(totalSec / 60)%60;var formattedTime=$('<a>').attr('title','<div><b>Time remaining until you can send</b></div>').text(s('%1h %2m',hours,minutes));table.appendToCell(-1,'send-links',formattedTime);}else{table.appendToCell(-1,'send-links',generateSendLink(pid));}};function prettyDropTypes(key){return logsMetadata.logTypes[key]||logTypes[key]||"Unknown";}
var refreshTable=function(sortingType){processLogs(false).then(function(){var players=[];$.each(friends,function(pid){try{players.push({'id':pid,'name':friends[pid].name,'total':playerLogs[pid].total,'timeUntilReady':timeUntilSesReady(pid)});}catch(e){players.push({'id':pid,'name':friends[pid].name,'total':0,'timeUntilReady':timeUntilSesReady(pid)});}});players.sort(function(a,b){switch(sortingType){case"totalUp":return b.total-a.total;break;case"totalDown":return a.total-b.total;break;case"receivedUp":return a.timeUntilReady-b.timeUntilReady;break;case"receivedDown":return b.timeUntilReady-a.timeUntilReady;break;case"nameUp":return(a.name.toLowerCase()>b.name.toLowerCase())?1:-1;break;case"nameDown":return(a.name.toLowerCase()<b.name.toLowerCase())?1:-1;break;}});var friendsTable=new west.gui.Table();friendsTable.addColumn('remove-link').setId('twbf-table');var nameColHead=$("<a>").html('<img src="//westzzs.innogamescdn.com/images/icons/user.png" alt="" />&nbsp;'+'Name').click(function(){refreshTable("name"+sortingDirection);(sortingDirection==="Up")?sortingDirection="Down":sortingDirection="Up";});friendsTable.addColumn('player-names').appendToCell('head','player-names',nameColHead);var receivedColHead=$("<a>").text('Received').click(function(){refreshTable("total"+sortingDirection);(sortingDirection==="Up")?sortingDirection="Down":sortingDirection="Up";});friendsTable.addColumn('total-received').appendToCell('head','total-received',receivedColHead);var sendColHead=$("<a>").text('Send').click(function(){refreshTable("received"+sortingDirection);(sortingDirection==="Up")?sortingDirection="Down":sortingDirection="Up";});friendsTable.addColumn('send-links').appendToCell('head','send-links',sendColHead);for(var i=0;i<players.length;i++)
appendPlayerToTable(friendsTable,players[i].id);var moreData="";$.each(dropTypeLogs,function(key){moreData+='<b>'+prettyDropTypes(key)+':</b> '+dropTypeLogs[key]+', ';});$('#twbf-table').html(friendsTable.divMain);});}
function openWindow(){processLogs(false).then(function(){var players=[];$.each(friends,function(pid){try{players.push({'id':pid,'name':friends[pid].name,'total':playerLogs[pid].total,'timeUntilReady':timeUntilSesReady(pid)});}catch(e){players.push({'id':pid,'name':friends[pid].name,'total':0,'timeUntilReady':timeUntilSesReady(pid)});}});players.sort(function(a,b){return a.timeUntilReady-b.timeUntilReady;});var windowContent=new west.gui.Scrollpane();var friendsTable=new west.gui.Table();friendsTable.addColumn('remove-link').setId('twbf-table');var nameColHead=$("<a>").html('<img src="//westzzs.innogamescdn.com/images/icons/user.png" alt="" />&nbsp;'+'Name').click(function(){refreshTable("name"+sortingDirection);(sortingDirection==="Up")?sortingDirection="Down":sortingDirection="Up";});friendsTable.addColumn('player-names').appendToCell('head','player-names',nameColHead);var receivedColHead=$("<a>").text('Received').click(function(){refreshTable("total"+sortingDirection);(sortingDirection==="Up")?sortingDirection="Down":sortingDirection="Up";});friendsTable.addColumn('total-received').appendToCell('head','total-received',receivedColHead);var sendColHead=$("<a>").text('Send').click(function(){refreshTable("received"+sortingDirection);(sortingDirection==="Up")?sortingDirection="Down":sortingDirection="Up";});friendsTable.addColumn('send-links').appendToCell('head','send-links',sendColHead);for(var i=0;i<players.length;i++)
appendPlayerToTable(friendsTable,players[i].id);var moreData="";$.each(dropTypeLogs,function(key){moreData+='<b>'+prettyDropTypes(key)+':</b> '+dropTypeLogs[key]+', ';});windowContent.appendContent(friendsTable.divMain);windowContent.appendContent(moreData);wman.open('twbf','twbf','noreload').setTitle('TW Best Friends').appendToContentPane(windowContent.divMain).setMiniTitle('TW Best Friends - Sending currency made easier!').setSize('500','420');});}
var styling=$('<style></style>').text('.remove-link { width:20px; } .player-names { width:175px; } .total-received { text-align:center; width:50px; } .send-links { text-align:right; width:170px }');$('head').append(styling);function initialiseCounter(){$('.xsht_custom_unit_counter').remove();var evAvailable=$('<span></span>').attr('id','twbf_value').text(getSesReadyCount());var evLimit=$('<span></span>').attr('class','twbf_limit').text(' / '+getFriendCount()).css({'color':'lightgray','font-size':'11px'});var timeUntilCanSend=getSesSmallestTimer();var hours=parseInt(timeUntilCanSend / 3600)%24;var minutes=parseInt(timeUntilCanSend / 60)%60;var formattedTime=$('<span></span>').css({'position':'absolute','left':'3px','top':'1px','font-size':'12px'}).attr({'title':'<div><b>Time remaining until you can send</b></div>','id':'twbf_timer'}).text(' ');var evValue=$('<div></div>').attr('class','value').css({'position':'absolute','left':'32px','top':'3px','width':'105px','height':'25px','line-height':'25px','pading':'0 5px','color':'#f8c57c','font-size':'13pt','text-align':'right','user-select':'none','background':'url("https://westzzs.innogamescdn.com/images/interface/custom_unit_counter_sprite.png?2") no-repeat 0 -36px','z-index':'1'}).html(evAvailable).append(evLimit);evValue.append(formattedTime);if(timeUntilCanSend!=0){$('#twbf_timer').text(s('%1h %2m',hours,minutes));setTimeout(updateCounterTimer,(timeUntilCanSend%60+1)*1000);}
...
function updateCounterTimer(){var timeUntilCanSend=getSesSmallestTimer();var hours=parseInt(timeUntilCanSend / 3600)%24;var minutes=parseInt(timeUntilCanSend / 60)%60;if(timeUntilCanSend==0){updateCounter();$('#twbf_timer').text(' ');}else{$('#twbf_timer').text(s('%1h %2m',hours,minutes));setTimeout(updateCounterTimer,(timeUntilCanSend%60+1)*1000);}}
initialiseScript();function registerToWestApi(){scriptInfo="What are you doing here?";window.scriptyscript={script:TheWestApi.register('twbf','The West Best Friends','2',Game.version.toString(),'xShteff, Diggo11, Leones/Slygoxx','https://github.com/xShteff'),setGui:function(){this.script.setGui(scriptInfo);},init:function(){this.setGui();}};window.scriptyscript.init();}}).toString()+')()';document.head.appendChild(script);